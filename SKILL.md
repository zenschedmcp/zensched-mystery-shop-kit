...
| Follow-up only when X | any field with `show_if: {"field": <identifier of an earlier select/multi_select>, "op": "equals" \| "not_equals" \| "contains" \| "is_empty" \| "is_not_empty", "value": <option key>, "action": "show"}` | Sources must be an earlier `select` / `multi_select`. Honored on the phone. Hidden required fields do not block submit. |
